...
def get_random_problems(batch_size, problem_size, distribution='uniform'):
    if distribution != "mix":
        return _get_random_problems(batch_size, problem_size, distribution)
    else:
        target_distribution = random.choice(["uniform", "cluster", "mixed"])
        return _get_random_problems(batch_size, problem_size, target_distribution)

def _get_random_problems(batch_size, problem_size, distribution='tsplib', num_modes=2, cdist=5):
    if distribution == "uniform":
        # problems = np.random.uniform(0, 1, [batch_size, problem_size, 2])
        problems = torch.rand(size=(batch_size, problem_size, 2))
    elif distribution == "gaussian_mixture_2_5":
        problems = generate_gaussian_mixture_tsp(batch_size, problem_size, num_modes=2, cdist=5)
    elif distribution == "gaussian_mixture_3_10":
        problems = generate_gaussian_mixture_tsp(batch_size, problem_size, num_modes=3, cdist=10)
    elif distribution == "gaussian_mixture_7_50":
        problems = generate_gaussian_mixture_tsp(batch_size, problem_size, num_modes=7, cdist=50)
    ...
    elif distribution in ["tsplib"]:
        problems, names = generate_tsp_dist(batch_size, problem_size, distribution)
...
def generate_tsp_dist(n_samples, n_nodes, distribution):
    ...
    if distribution == "cluster":  # time-consuming
        ...
        file_names = glob.glob("data/tspliball/*.tsp") if distribution == "tsplib" else glob.glob("../data/CVRP/cvrplib/Vrp-Set-X/*.vrp")
        data = []
        name = []
    ...
    if distribution not in  ["uniform_rectangle", "mixed"]:
...
#     nodes = get_random_problems(1, 100)
#     plt.scatter(nodes[0, :, 0], nodes[0, :, 1])
#     plt.show()